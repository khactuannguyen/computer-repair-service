...
export async function updateRepairOrder(orderId: string, formData: FormData) {
  ...
  try {
    const updateData: any = {
      ...
      estimatedCost: formData.get("estimatedCost") ? Number(formData.get("estimatedCost")) : undefined,
      finalCost: formData.get("finalCost") ? Number(formData.get("finalCost")) : undefined,
      estimatedCompletionDate: formData.get("estimatedCompletionDate") || undefined,
    }

    const assignedToId = formData.get("assignedTo")
    if (assignedToId && assignedToId !== "none") {
      updateData.assignedTo = new mongoose.Types.ObjectId(assignedToId as string)
    } else {
    ...
    const updatedOrder = await RepairOrder.findByIdAndUpdate(orderId, updateData, { new: true, runValidators: true })
...
export async function addInternalNote(orderId: string, formData: FormData) {
  ...
  try {
    ...
    const session = await getSession()
    const noteWithUser = `${session?.name} (${new Date().toLocaleString("vi-VN")}): ${note}`
...
export async function getRepairOrderByTracking(trackingCode: string, phone: string) {
  await connectToDatabase()
  ...
  try {
    ...
    if (!order) {
      return { error: "Không tìm thấy đơn sửa chữa với mã theo dõi và số điện thoại này" }
    }

    if (order.assignedTo) {
      const technician = await User.findById(order.assignedTo).select("name").lean()
      if (technician) {
