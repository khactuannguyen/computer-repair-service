    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="container mx-auto max-w-4xl px-4">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-4">Tra cứu đơn sửa chữa</h1>
          <p className="text-gray-600">Nhập mã đơn hàng và số điện thoại để kiểm tra trạng thái sửa chữa của bạn</p>
        </div>
        ...
        <Card className="mb-8">
          ...
          <CardContent className="space-y-4">
            ...
            <Button onClick={handleSearch} disabled={loading} className="w-full bg-yellow-500 hover:bg-yellow-600">
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900 mb-3">Tiến độ sửa chữa</h3>
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">Đơn hàng đã được tạo</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center ${
                          ["in_progress", "completed"].includes(order.status) ? "bg-green-100" : "bg-gray-100"
                        }`}
                      >
                        <Package
                          className={`h-4 w-4 ${
                            ["in_progress", "completed"].includes(order.status) ? "text-green-600" : "text-gray-400"
                          }`}
                        />
                      </div>
                      <div>
                        <p className="text-sm font-medium">Đang sửa chữa</p>
                        <p className="text-xs text-gray-500">Kỹ thuật viên đang xử lý</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      >
                        <CheckCircle
                          className={`h-4 w-4 ${order.status === "completed" ? "text-green-600" : "text-gray-400"}`}
                        />
                      </div>
                      <div>
                        <p className="text-sm font-medium">Hoàn thành</p>
                        <p className="text-xs text-gray-500">Sẵn sàng giao cho khách hàng</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {order.estimatedCompletionDate && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <h4 className="font-semibold text-blue-900 mb-1">Thời gian dự kiến hoàn thành</h4>
                  <p className="text-blue-700">{new Date(order.estimatedCompletionDate).toLocaleDateString("vi-VN")}</p>
                </div>
              )}
